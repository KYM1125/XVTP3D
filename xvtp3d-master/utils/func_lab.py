...
def get_neighbour_3Dpoints(points, topk_ids=None, neighbour_dis=2):
    '''
        该函数的作用是将3D点云投影到2D平面上，然后根据2D平面的点云，获取周围的点云，防止点云的稀疏性
        :param points: 3D点云坐标
        :param topk_ids: 选取的点云的索引，后续可以对points[topk_ids]特殊处理
        :param neighbour_dis: 邻域距离,
    '''
    # grid = np.zeros([300, 300], dtype=int)
    grid = {}
    for fake_idx, point in enumerate(points):
        x, y = round(float(point[0])), round(float(point[1])) # 3D点云投影到2D平面上
        z = 0

        # not compatible argo，获取周围的点云，不适合argo数据集
        for i in range(-neighbour_dis, neighbour_dis + 1):
            for j in range(-neighbour_dis, neighbour_dis + 1):
                grid[(x + i, y + j, z)] = 1
    points = list(grid.keys())
